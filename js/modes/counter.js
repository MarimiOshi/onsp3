...
const CounterMode = {
    ...
    addEventListeners: function() {
        ...
        // カウンターアイテムへのイベントリスナーは renderCounterGrid 内で設定
    },

    loadCounts: function() {
        this.state.counts = this.dependencies.storage.loadCounterData();
        // config.members に含まれない古いメンバーのデータを削除する処理 (オプション)
    ...
    renderCounterGrid: function() {
        ...
        this.config.members.forEach(member => {
            ...
            const img = this.dependencies.domUtils.createElement('img', {
                src: `images/count/${memberName}.jpg`, // カウンター用画像パス
                alt: memberName,
                loading: 'lazy'
            });
            ...
            // メンバーカラーを適用 (config.jsのmember.colorを使用)
            if (member.color) {
    ...
    handleCounterClick: function(event) {
        ...
        setTimeout(() => {
            this.dependencies.domUtils.removeClass(itemElement, 'clicked');
        }, 150);
    },
    ...
    /**
     * 指定されたメンバーのカウントを1増やす (外部からの呼び出し用)
    ...
    activate: function() {
        ...
        this.loadCounts(); // アクティブ化時に最新のカウントを読み込む
        this.renderCounterGrid(); // グリッドを再描画して最新のカウントを反映
