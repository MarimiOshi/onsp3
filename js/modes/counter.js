...
const CounterMode = {
    elements: {
        section: null,
        decreaseModeButton: null,
        counterGrid: null,
    },
    state: {
        isActive: false,
        isDecreaseMode: false,
        counts: {}, // { memberName: count }
    },
    config: {
        members: [], // app.js から注入
    },
    dependencies: {
        app: null, // Appインスタンスへの参照 (オプション)
        storage: null,
        utils: null,
        domUtils: null,
    },
    ...
    init: function(appInstance, initialConfig) {
        this.dependencies.app = appInstance; // Appインスタンスを保持
        ...
        if (initialConfig.members) {
            ...
            console.error("CounterMode: initialConfig.members is missing!");
        }

        this.loadCounts();
        this.renderCounterGrid(); // 初期描画
        this.addEventListeners();
        this.updateDecreaseModeButtonUI();
    ...
    addEventListeners: function() {
        if (this.elements.decreaseModeButton) {
            this.dependencies.domUtils.on(this.elements.decreaseModeButton, 'click', () => this.toggleDecreaseMode());
        }
        // カウンターアイテムへのイベントリスナーは renderCounterGrid 内で動的に設定
    },

    loadCounts: function() {
        this.state.counts = this.dependencies.storage.loadCounterData();
        // config.members に含まれない古いメンバーのデータを削除する処理
        const validMemberNames = new Set((this.config.members || []).map(m => m.name));
        for (const memberName in this.state.counts) {
            if (!validMemberNames.has(memberName)) {
                delete this.state.counts[memberName];
            }
        }
    },

    saveCounts: function() {
        this.dependencies.storage.saveCounterData(this.state.counts);
    },

    renderCounterGrid: function() {
        if (!this.elements.counterGrid || !this.config.members) {
            console.error("CounterMode: Cannot render grid, element or members data missing.");
            return;
        }
        this.dependencies.domUtils.empty(this.elements.counterGrid);

        if (this.config.members.length === 0) {
            this.elements.counterGrid.appendChild(
                this.dependencies.domUtils.createElement('p', { class: 'loading-message' }, ['メンバー情報がありません。'])
            );
            return;
        }
        ...
        this.config.members.forEach(member => {
            ...
            const count = this.state.counts[memberName] || 0;

            const img = this.dependencies.domUtils.createElement('img', {
                src: `images/count/${memberName}.jpg`,
                alt: memberName,
                loading: 'lazy'
            });
            this.dependencies.domUtils.on(img, 'error', (e) => {
                if (e.target) {
            ...
            if (member.color) {
                img.style.borderColor = member.color;
                countSpan.style.color = member.color;
            }
    ...
    handleCounterClick: function(event) {
        ...
        const memberName = itemElement.dataset.memberName;

        if (!memberName) return;

        let currentCount = this.state.counts[memberName] || 0;
        if (this.state.isDecreaseMode) {
            if (currentCount > 0) currentCount--;
        } else {
            currentCount++;
        }
        this.state.counts[memberName] = currentCount;
        this.saveCounts();
        ...
        setTimeout(() => {
            if (itemElement) this.dependencies.domUtils.removeClass(itemElement, 'clicked');
        }, 150);
    },

    toggleDecreaseMode: function() {
        this.state.isDecreaseMode = !this.state.isDecreaseMode;
        this.updateDecreaseModeButtonUI();
    },

    updateDecreaseModeButtonUI: function() {
        if (this.elements.decreaseModeButton) {
            this.dependencies.domUtils.setText(
                this.elements.decreaseModeButton,
                `減らすモード ${this.state.isDecreaseMode ? 'ON' : 'OFF'}`
            );
            this.dependencies.domUtils.toggleClass(
                this.elements.decreaseModeButton,
                'decrease-mode-active',
                this.state.isDecreaseMode
            );
        }
    },

    incrementCount: function(memberName) {
        if (!(this.config.members || []).find(m => m.name === memberName)) {
            console.warn(`Counter: Member "${memberName}" not found for increment.`);
            return;
        }
        let currentCount = this.state.counts[memberName] || 0;
        currentCount++;
